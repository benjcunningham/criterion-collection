...
def create_path(url: str, path: str = "") -> str:
    """
    ...
    Examples:
        >>> create_path("https://www.criterion.com/shop/browse/list")
        'list'
...
def download_page(url: str, path: Optional[str] = None) -> str:
    """
    ...
    Returns:
        str: If `path` is None, the resulting file as a string. Otherwise the path
        to the downloaded file on disk.

    Examples:
        >>> download_page("https://www.criterion.com/shop/browse/list", "/tmp")
        '/tmp/list'
    ...
    if path is None:
        ...
        path_or_text = create_path(url, path)
